...
const clickHandler = (event) => {
    ...
    var productCounts = {};
    function calculateDiscountedPayment( product, price ) {
        if ( productCounts[product] ) {
            productCounts[product]++;
        }
        if ( productCounts[product] % 3 == 0 ) {
            return price * 0.5;
        }
        else{
            return price;
    ...
    const discountedPrice = calculateDiscountedPayment(productId,productPrice);

    price.textContent =
        (parseFloat(price.textContent) + discountedPrice).toFixed(2) + ' $';
};
